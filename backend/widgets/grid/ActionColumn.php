...
class ActionColumn extends Column
{
    ...
    {
        $this->initDefaultButton('view', 'mdi mdi-eye');
        $this->initDefaultButton('update', 'mdi mdi-square-edit-outline');
        
        $this->initDefaultButton(
            'delete',
            ArrayHelper::getValue($this, 'grid.filterModel.state') == 'archive' ? 'mdi mdi-delete-restore' : 'mdi mdi-delete',
            [
            ...
            $this->buttons[$name] = function ($url, $model, $key) use ($name, $iconName, $additionalOptions) {
                switch ($name) {
                    ...
                    case 'update':
                        ...
                        break;
                    case 'delete':
                        $title = ArrayHelper::getValue($this, 'grid.filterModel.state') == 'archive' ? 'Восстановить' : 'Удалить';
    ...
    protected function renderDataCellContent($model, $key, $index)
    {
        $buttons = preg_replace_callback('/\\{([\w\-\/]+)\\}/', function ($matches) use ($model, $key, $index) {
            ...
            if (isset($this->visibleButtons[$name])) {
                ...
                $route = $this->createUrl($name, $model, $key, $index);
                $route = str_replace("/admin/$lang/", null, $route);
                $route = explode('?', $route)[0];
                
                $isVisible = Yii::$app->services->permissions->isAllowedByRoute($route);
            }
