...
class GridView extends BaseListView
{
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $tableOptions = ['class' => 'table table-striped mb-2'];
    /**
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $headerRowOptions = ['class' => 'labels'];
    /**
    ...
    public function init()
    {
        ...
        $this->layout = $this->layout ?: '{items}' . Html::tag(
            'div',
            Html::tag('div', '{summary}', ['class' => 'col-sm-12 col-md-5'])
            . Html::tag('div', '{pager}', ['class' => 'col-sm-12 col-md-7']),
            ['class' => 'row mt-3']
        );
        ...
        $this->buttons = $this->buttons ?? [
            ...
            'delete' => ArrayHelper::getValue($this, 'filterModel.state') == 'archive' ? [
    ...
    public function renderItems()
    {
        ...
        $per_page_block =  Html::dropDownList('per-page', Yii::$app->request->get('per-page'), [
            ...
            'class' => 'form-select form-select-sm mx-1 grid-view-per-page',
        ]);
        
        $result .= Html::tag('div', Yii::t('app', 'Показывать {dropdown} записей', ['dropdown' => $per_page_block]), ['class' => 'clearfix mb-3']);
        $result .= Html::tag('div', $table, ['class' => 'table-responsive']);
            ...
            foreach ($this->buttons as $key => $button) {
                ...
                if (is_array($button)) {
                    $buttons[] = Html::submitButton(
                        ($icon_class ? Html::tag('i', null, ['class' => "$icon_class me-2"]) : null) . $label,
                        [
                            'formaction' => Url::to($button['action']),
                            'onclick' => $button['onclick'] ?? null,
                            'data-trigger' => $button['data-trigger'] ?? null,
                            'class' => 'btn btn-' . ArrayHelper::getValue($button, 'css_class'),
                        ]
                    );
                } else {
                    $buttons[] = $button;
