...
<table class="table table-bordered table-striped table-relations tests-validation-table">
    ...
    <tbody data-toggle="sortable">
        <?php foreach ($options['options'] as $key => $value) { ?>
            <tr class="<?= $key === '__key__' ? 'table-new-relation' : null ?>" data-table="options-radio-<?= $model->id ?? 0 ?>">
                <td>
    ...
    <tfoot>
        <tr>
            <td colspan="4">
                <button type="button" class="btn btn-success w-100 btn-add" data-table="options-radio-<?= $model->id ?? 0 ?>">
